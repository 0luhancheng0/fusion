    ...
    def scatter_plot(self):
        ...
        for x, y, model in zip(x_positions, test_accuracies, models):
            ...
            ax.annotate(
                f'{y:.3f}',
                (x, y),
                xytext=(0, -20) if x != len(models) - 1 else (0, 10),  # 20 points below except last point
        ...
        if has_hard_lp:
            for x, y in zip(x_positions, lp_hard_aucs):
                ax2.annotate(
                    f'{y:.3f}',
                    (x, y),
                    xytext=(0, -15),  # 35 points below (further down than standard LP)
                    textcoords='offset points',
                    ha='center',
                    fontsize=9,
                    color='#2ca02c',
        ...
        ax.legend(lines, labels, loc='lower left', frameon=True, framealpha=0.9)
